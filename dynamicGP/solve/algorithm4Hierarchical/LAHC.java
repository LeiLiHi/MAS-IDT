...
import org.lileischeduler.dynamicGP.solve.operator.NeighborOperator;
...
public class LAHC {
    ...
    public static void run(Satellite satellite, int historyLength, int maxIterations) {
        // 初始化当前解和最佳解
        if (satellite.currentTargets.isEmpty()) return;
        // 初始化一个随机顺序的路径作为当前解决方案
        List<Target> currentSolution = new ArrayList<>(satellite.currentTargets);
        ...
        // LAHC主循环
        for (int tt = 0; tt < maxIterations; tt++) {
            // 创建邻域解
        ...
        new Insert().decodeTarget(bestSolution, satellite.scheduledTargets, satellite.currentTargets);
    }
