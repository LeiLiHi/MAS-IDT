...
public class HALNS {
    ...
    public void run(double[] para, BufferedWriter writer, LoadSatellite loadSatellite, LoadStation loadStation, List<Target> targets) throws IOException {
        ...
        double stepAlns = para[1];              // ALNS的迭代次数
        ...
        for (int i = 0; i < iteration; i++) {
            ...
            for (Satellite satellite : loadSatellite.satelliteList) {
                ALNS.AdaptiveLargeNeighborSearch(satellite, (int) (satellite.scheduledTargets.size() * 0.1), (int) stepAlns);
            }
            // 3. 数传解码
            loadSatellite.satelliteList.sort(Comparator.comparingInt(s -> ((Satellite) s).scheduledTargets.size()).reversed());
            double[] finishedNum = DecodeDownload.decode(loadSatellite.satelliteList, loadStation.stationList, true);
            ...
            // 6. 随机删除卫星的任务
            for (int j = 0; j < loadSatellite.satelliteList.size() * 0.2; j++) {
                Satellite satellite = loadSatellite.satelliteList.get(random.nextInt(loadSatellite.satelliteList.size()));
                int k = 1;
                while (k <= satellite.scheduledTargets.size() * 0.2 ) {
                    if (satellite.scheduledTargets.size() - 2 < 0) break;
                    Target target = satellite.scheduledTargets.get(random.nextInt(satellite.scheduledTargets.size() - 2) + 1);
